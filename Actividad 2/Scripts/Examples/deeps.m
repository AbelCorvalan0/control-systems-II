...
% Condiciones iniciales - MODIFICADO: comenzar exactamente en pi
alfa(1) = pi; colorc = '.g'; colorl = 'g';
...
% Punto de operación MODIFICADO: queremos llevar el ángulo a 0 (no a pi)
xOP = [0; 0; 0; 0];  % Cambiado de [0;0;pi;0] a [0;0;0;0]
...
% Ajuste de matrices de ponderación MODIFICADO: mayor peso en el ángulo
Qa = diag([1e1 1e1 1e6 1e6 1e-3]);  % Aumentado peso en alfa y omega
Ra = 1e1;
Q = Qa(1:4,1:4); R = Ra;
...
Mat_Bdual = Mat_C';
Mat_Cdual = Mat_B';
Qdual = diag([1e1 1 1e1 1]); Rdual = 1e-3*eye(2);
...
t = 0; 
x = [0; 0; alfa(1); 0];  % Comienza con alfa = pi
p(1) = x(1); p_p(1) = x(2); alfa(1) = x(3); omega(1) = x(4);
tl = (0:KMAX-1)*Ts;
x_hat = [0; 0; 0; 0];  % Estimación inicial del observador
h = Ts; ref = 0;  % MODIFICADO: referencia a 0 (queremos alfa -> 0)
tita_pp = 0; 
um = 0; % Zona muerta .1
psi = zeros(KMAX+1,1);  % Inicializar psi
...
t = 0:h:T;
figure(1); hold on;
subplot(3,2,1); plot(t, alfa, colorc); grid on; 
title('\phi_t', 'FontSize', TamanioFuente); ylabel('Rad'); hold on;
subplot(3,2,2); plot(t, omega, colorc); grid on;
title('$\dot{\phi_t}$', 'Interpreter', 'latex', 'FontSize', TamanioFuente); hold on;
subplot(3,2,3); plot(t, p, colorc); grid on; 
title('\delta_t', 'FontSize', TamanioFuente); hold on;
subplot(3,2,4); plot(t, p_p, colorc); grid on;
title('$\dot{\delta_t}$', 'Interpreter', 'latex', 'FontSize', TamanioFuente); hold on;
subplot(3,1,3); plot(t(1:end-1), u, colorc); grid on;
